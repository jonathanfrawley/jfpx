...
/**
  ...
  */
__device__ void sphereSphereCollision(float3* contactNormal,
    float4* contactPointPenetration,
    int* valid, float4 currentSphere,
    float4 otherSphere)
{
    float3 midline;
...
/**
  * Called at the beginning of checking all of the spheres as this is the only
 * case when the
  * number of spheres tested is not uniform.
  */
__device__ void tileSphereSphereCalculationStart(
    float3* currentContactNormal, float4* currentContactPointPenetration,
    int* valid, float4 currentSphere, float4 sharedSpheres[BLOCK_SIZE],
    unsigned amount)
{
    int i;
    ...
    for (i = 0; i < amount; i++) {
        sphereSphereCollision(&(currentContactNormal[i]),
            &(currentContactPointPenetration[i]), &(valid[i]),
            currentSphere, sharedSpheres[threadIdx.x + 1 + i]);
    }
    return;
}
...
/**
  ...
  * in shared memory.
  */
__device__ void tileSphereSphereCalculation(
    float3* currentContactNormal, float4* currentContactPointPenetration,
    int* valid, float4 currentSphere, float4 sharedSpheres[BLOCK_SIZE])
{
    int i;
 ...
 */
__device__ unsigned computeContactRowIndex(unsigned n_Spheres,
    unsigned sphere_row)
{
 ...
 */
__device__ unsigned computeContactNumCols(unsigned sphere_row)
{
 ...
 */
__global__ void calculateContacts(float4* devContactNormal,
    float4* devContactPointPenetration,
    int4* devContactValidBodies,
    float4* devSphere, unsigned numSpheres)
{
    ...
    __syncthreads();
    // Do the first loop, as it is slightly different, not dealing with square
    // blocks, triangular ones instead
    tileSphereSphereCalculationStart(
        &(currentContactNormal[i]), &(currentContactPointPenetration[i]),
        &(currentContactValid[i]), mySphere, sharedSpheres,
        amountContactsFirstBlock); // don't skip too far ahead, triangular array
    // first
    __syncthreads();

    // Start at our blockID, this ensures we don't check ones
    // that have been done by other blocks
    for (i = amountContactsFirstBlock, tile = (blockIdx.x + 1); i < numCols;
         i += BLOCK_SIZE, tile++) {
...
__host__ void copySpheresToFloat4s(float4* sphereFloat4s,
...
__host__ void generateContacts(jfContactStruct contactStructs[N_CONTACTS],
    ...
    jfContactStruct* contactStructs)
{
    ...
    // Call Kernel
    calculateContacts<<<grid, threads> > >(
        d_ContactNormal, d_ContactPointPenetration, d_ContactValidBodies,
        d_Sphere, N_SPHERES);
    // Get results from device
    cudaMemcpy(&contactNormal, d_ContactNormal, N_CONTACTS * sizeof(float4),
        cudaMemcpyDeviceToHost);

    cudaMemcpy(&contactPointPenetration, d_ContactPointPenetration,
        N_CONTACTS * sizeof(float4), cudaMemcpyDeviceToHost);

    cudaMemcpy(&contactValidBodies, d_ContactValidBodies,
        N_CONTACTS * sizeof(int4), cudaMemcpyDeviceToHost);
    ...
    generateContacts(contactStructs, contactNormal, contactPointPenetration,
        contactValidBodies, N_CONTACTS);
}
